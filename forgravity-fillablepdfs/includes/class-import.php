...
class Import {
	...
	private static function create_field( $props, $field_id ) {
		...
		switch ( $field['type'] ) {
			...
			case 'consent':
				$field['checkboxLabel']          = esc_html__( 'I agree to the privacy policy.', 'gravityforms' );
				$field['descriptionPlaceholder'] = esc_html__( 'Enter consent agreement text here.  The Consent Field will store this agreement text with the form entry in order to track what the user has consented to.', 'gravityforms' );
