...
namespace ForGravity\Fillable_PDFs\Plugin_Skeleton;
...
class EDD_SL_Plugin_Updater
{
    ...
    public function show_update_notification($file, $plugin)
    {
        ...
        if (!current_user_can('update_plugins')) {
            ...
            echo ' ';
            \printf(__('%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'cosmicgiant'), '<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url($changelog_link) . '">', esc_html($update_cache->response[$this->name]->new_version), '</a>', '<a target="_blank" class="update-link" href="' . esc_url(wp_nonce_url($update_link, 'upgrade-plugin_' . $file)) . '">', '</a>');
        } else {
            \printf(' %1$s%2$s%3$s', '<a target="_blank" class="update-link" href="' . esc_url(wp_nonce_url($update_link, 'upgrade-plugin_' . $file)) . '">', esc_html__('Update now.', 'cosmicgiant'), '</a>');
        }
    ...
    private function get_version_from_remote($action)
    {
        ...
        if ($action === 'plugin_latest_version') {
            ...
            foreach (get_plugins() as $key => $plugin) {
                ...
                $plugins[] = array('name' => $plugin['Name'], 'slug' => $slug, 'version' => $plugin['Version'], 'is_active' => \in_array($key, $active_plugins));
            }
            $api_params['stats'] = array('gf' => \GFForms::$version, 'wp' => get_bloginfo('version'), 'php' => \phpversion(), 'mysql' => $wpdb->db_version(), 'plugins' => \json_encode($plugins), 'im' => is_multisite(), 'lang' => get_locale());
        }
        /**
