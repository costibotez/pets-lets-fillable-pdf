...
namespace ForGravity\Fillable_PDFs\Masbug\Flysystem;

use ForGravity\Fillable_PDFs\Google\Service\Drive;
use ForGravity\Fillable_PDFs\Google\Service\Drive\DriveFile;
use ForGravity\Fillable_PDFs\Google\Service\Drive\FileList;
use ForGravity\Fillable_PDFs\Google\Service\Drive\Permission;
use ForGravity\Fillable_PDFs\GuzzleHttp\Psr7\Utils;
use ForGravity\Fillable_PDFs\League\Flysystem\Adapter\AbstractAdapter;
use ForGravity\Fillable_PDFs\League\Flysystem\AdapterInterface;
use ForGravity\Fillable_PDFs\League\Flysystem\Config;
use ForGravity\Fillable_PDFs\League\Flysystem\FileNotFoundException;
use ForGravity\Fillable_PDFs\League\Flysystem\Util;
use ForGravity\Fillable_PDFs\Psr\Http\Message\RequestInterface;
use ForGravity\Fillable_PDFs\Psr\Http\Message\StreamInterface;
...
class GoogleDriveAdapter extends AbstractAdapter
{
     ...
     * @var array
     */
    protected static $defaultOptions = ['spaces' => 'drive', 'useHasDir' => \false, 'useDisplayPaths' => \true, 'usePermanentDelete' => \false, 'useSinglePathTransaction' => \false, 'publishPermission' => ['type' => 'anyone', 'role' => 'reader', 'withLink' => \true], 'appsExportMap' => ['application/vnd.google-apps.document' => 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.google-apps.spreadsheet' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.google-apps.drawing' => 'application/pdf', 'application/vnd.google-apps.presentation' => 'application/vnd.openxmlformats-officedocument.presentationml.presentation', 'application/vnd.google-apps.script' => 'application/vnd.google-apps.script+json', 'default' => 'application/pdf'], 'parameters' => [], 'driveId' => null, 'sanitize_chars' => [
        // sanitize filename
    ...
    public function __construct($service, $root = null, $options = [])
    {
        ...
        if (isset($this->options['teamDriveId'])) {
            ...
            if ($this->useDisplayPaths && $root !== null) {
                // get real root id
                $this->root = $this->toSingleVirtualPath($root, \false, \true, \true, \true);
                // reset cache
    ...
    {
        $operations = ['files.copy', 'files.create', 'files.delete', 'files.trash', 'files.get', 'files.list', 'files.update', 'files.watch'];
        $clean = [];
    ...
    public function write($path, $contents, Config $config)
    {
        ...
        if ($this->useDisplayPaths) {
            try {
                $virtual_path = $this->toVirtualPath($path, \true, \false);
                $updating = \true;
                // destination exists
            } catch (FileNotFoundException $e) {
                $updating = \false;
                [$parentDir, $fileName] = $this->splitPath($path, \false);
                $virtual_path = $this->toSingleVirtualPath($parentDir, \false, \true, \true, \true);
                if ($virtual_path === '') {
            ...
            if ($updating && \is_array($virtual_path)) {
                ...
                if (\count($virtual_path) > 1) {
                    ...
                    $this->delete_by_id(\array_map(function ($p) {
    ...
    public function rename($path, $newpath)
    {
        $this->refreshToken();
        if ($this->useDisplayPaths) {
            $path = $this->toVirtualPath($path, \true, \true);
            $newpathDir = self::dirname($newpath);
            try {
                $toPath = $this->toVirtualPath($newpathDir, \false, \true);
            } catch (FileNotFoundException $e) {
                if ($this->createDir($newpathDir, new Config(), \true) === \false) {
                    return \false;
                }
                $toPath = $this->toVirtualPath($newpathDir, \false, \true);
            }
        ...
        $opts = ['fields' => self::FETCHFIELDS_GET];
    ...
    public function copy($path, $newpath)
    {
        $this->refreshToken();
        if ($this->useDisplayPaths) {
            $srcId = $this->toVirtualPath($path, \false, \true);
            $newpathDir = self::dirname($newpath);
            $toPath = $this->toSingleVirtualPath($newpathDir, \false, \false, \true, \true);
            if ($toPath === \false) {
                return \false;
            }
            if ($toPath === '') {
        ...
        $newFile = $this->service->files->copy($srcId, $file, $this->applyDefaultParams(['fields' => self::FETCHFIELDS_GET], 'files.copy'));
        if ($newFile instanceof DriveFile) {
            ...
            if ($this->useSinglePathTransaction && $this->useDisplayPaths) {
                $this->cachedPaths[\trim($newpathDir . '/' . $fileName, '/')] = $id;
            }
    ...
    public function createDir($dirname, Config $config, $internalCall = \false)
    {
        ...
        if ($meta !== \false) {
            return ['path' => $meta['path'], 'filename' => $meta['filename'], 'extension' => $meta['extension']];
        }
        [$pdir, $name] = $this->splitPath($dirname, \false);
        if ($this->useDisplayPaths) {
            if ($pdir !== $this->root) {
                $pdir = $this->toSingleVirtualPath($pdir, \false, \false, \true, \true);
                // recursion!
                if ($pdir === \false) {
                    return \false;
                }
        ...
        if ($folder !== null) {
            ...
            $result = ['path' => Util::normalizeDirname($pdir) . '/' . ($this->useDisplayPaths ? $name : $itemId), 'filename' => $path_parts['filename'], 'extension' => $path_parts['extension']];
            return $result;
        }
        return \false;
    }
    ...
    public function has($path)
    {
        ...
        return $this->getFileObject($path, \true) instanceof DriveFile;
    }
    /**
     * {@inheritdoc}
     */
    public function read($path)
    {
        $this->refreshToken();
        if ($this->useDisplayPaths) {
            $fileId = $this->toVirtualPath($path, \false, \true);
        } else {
    ...
    public function readStream($path)
    {
        $this->refreshToken();
        if ($this->useDisplayPaths) {
            $path = $this->toVirtualPath($path, \false, \true);
        }
        $redirect = null;
        if (\func_num_args() > 1) {
    ...
    public function listContents($directory = '', $recursive = \false)
    {
        ...
        if ($this->useDisplayPaths) {
            ...
            $elapsed = (\microtime(\true) - $time) * 1000.0;
            if (!\is_array($vp)) {
                $vp = [$vp];
            }
            $items = [];
            foreach ($vp as $path) {
                if (\DEBUG_ME) {
                    echo 'Converted display path to virtual path [' . \number_format($elapsed, 1) . 'ms]: ' . $path . "\n";
                }
    ...
    protected function setHasDir($targets, $object)
    {
        ...
        $opts = ['pageSize' => 1, 'orderBy' => 'folder,modifiedTime,name'];
        $paths = [];
        $client->setUseBatch(\true);
    ...
    protected function getItems($dirname, $recursive = \false, $maxResults = 0, $query = '')
    {
        ...
        $results = [];
        $parameters = ['pageSize' => $maxResults ?: 1000, 'fields' => self::FETCHFIELDS_LIST, 'orderBy' => 'folder,modifiedTime,name', 'spaces' => $this->spaces, 'q' => \sprintf('trashed = false and "%s" in parents', $itemId)];
        if ($query) {
                ...
                if ($fileObjs instanceof FileList) {
                    foreach ($fileObjs as $obj) {
                        ...
                        if ($result['type'] === 'dir') {
                            ...
                            if ($recursive) {
                                $results = \array_merge($results, $this->getItems($result['virtual_path'], \true, $maxResults, $query));
                            }
                        }
                    }
    ...
    public function getFileObject($path, $checkDir = \false)
    {
        ...
        try {
            ...
            if ($checkDir && $this->useHasDir) {
                /** @var RequestInterface $request */
                $request = $service->files->listFiles($this->applyDefaultParams(['pageSize' => 1, 'orderBy' => 'folder,modifiedTime,name', 'q' => \sprintf('trashed = false and "%s" in parents and mimeType = "%s"', $itemId, self::DIRMIME)], 'files.list'));
    ...
    protected function createDirectory($name, $parentId)
    {
        ...
        $obj = $this->service->files->create($file, $this->applyDefaultParams(['fields' => self::FETCHFIELDS_GET], 'files.create'));
    ...
    protected function upload($path, $contents, Config $config, $updating = null)
    {
        ...
        if ($size <= self::MAX_CHUNK_SIZE) {
            // one shot upload
            $params = ['data' => $stream, 'uploadType' => 'media', 'fields' => self::FETCHFIELDS_GET];
            if (!$updating) {
            ...
            // chunked upload
            $client = $this->service->getClient();
            $params = ['fields' => self::FETCHFIELDS_GET];
            $client->setDefer(\true);
            if (!$updating) {
                /** @var RequestInterface $request */
            ...
            $media = new StreamableUpload($client, $request, $mime, $stream, \true, self::MAX_CHUNK_SIZE);
        ...
        if (isset($obj) && $obj instanceof DriveFile) {
            ...
            if ($visibility = $config->get('visibility')) {
                if ($this->setVisibility($result['virtual_path'], $visibility, \true)) {
                    ...
                    if (!isset($this->cachedPaths[$path])) {
                        ...
                        if (!\is_array($this->cachedPaths[$path])) {
                            if ($itemId !== $this->cachedPaths[$path]) {
                                // convert to array
                                $this->cachedPaths[$path] = [$this->cachedPaths[$path], $itemId];
                                if (\DEBUG_ME) {
                ...
                if (!isset($this->cachedPaths[$fullPath])) {
                    ...
                    if (!\is_array($this->cachedPaths[$fullPath])) {
                        if ($itemId !== $this->cachedPaths[$fullPath]) {
                            // convert to array
                            $this->cachedPaths[$fullPath] = [$this->cachedPaths[$fullPath], $itemId];
                            if (\DEBUG_ME) {
    ...
    protected function makeFullVirtualPath($displayPath, $returnFirstItem = \false)
    {
        $paths = ['' => null];
        $tmp = '';
        $tokens = \explode('/', \trim($displayPath, '/'));
     ...
     * @throws FileNotFoundException
     */
    protected function toVirtualPath($displayPath, $makeFullVirtualPath = \true, $returnFirstItem = \false)
    {
        if ($displayPath === '' || $displayPath === '/' || $displayPath === $this->root) {
            return '';
        }
        $displayPath = \trim($displayPath, '/');
        // not needed
    ...
    protected function toDisplayPath($virtualPath)
    {
        ...
        $tokens = \explode('/', \trim($virtualPath, '/'));
    ...
    protected function toSingleVirtualPath($displayPath, $makeFullVirtualPath = \true, $can_throw = \true, $createDirsIfNeeded = \false, $is_dir = \false)
    {
        try {
            $path = $this->toVirtualPath($displayPath, $makeFullVirtualPath, \true);
        } catch (FileNotFoundException $e) {
            ...
            if ($subdir === '' || $this->createDir($subdir, new Config(), \true) === \false) {
                if ($can_throw) {
                    throw $e;
                }
                return \false;
    ...
    protected function canRequest($id, $is_full_req)
    {
        ...
        if (\time() - $this->requestedIds[$id]['time'] > self::FILE_OBJECT_MINIMUM_VALID_TIME) {
            return \true;
        }
        return \false;
        // not yet
    ...
    {
        $this->requestedIds[$id] = ['type' => (bool) $is_full_req, 'time' => \time()];
    }
    /**
    ...
    protected function sanitizeFilename($filename)
    {
        if (!empty($this->options['sanitize_chars'])) {
            $filename = \str_replace($this->options['sanitize_chars'], $this->options['sanitize_replacement_char'], $filename);
        }
        return $filename;
    }
    ...
    {
        $this->optParams = \array_merge_recursive(\array_fill_keys(['files.copy', 'files.create', 'files.delete', 'files.trash', 'files.get', 'files.list', 'files.update', 'files.watch', 'permissions.list'], ['supportsAllDrives' => \true]), $this->optParams);
    }
    /**
    ...
    public function setTeamDriveId($teamDriveId, $corpora = 'drive')
    {
        ...
        $this->optParams = \array_merge_recursive($this->optParams, ['files.list' => ['corpora' => $corpora, 'includeItemsFromAllDrives' => \true, 'driveId' => $teamDriveId]]);
