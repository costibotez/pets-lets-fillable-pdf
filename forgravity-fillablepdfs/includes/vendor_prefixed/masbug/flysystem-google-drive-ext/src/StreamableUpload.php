 ...
 */
use ForGravity\Fillable_PDFs\Google\Client;
use ForGravity\Fillable_PDFs\Google\Exception as GoogleException;
use ForGravity\Fillable_PDFs\Google\Http\REST;
use ForGravity\Fillable_PDFs\GuzzleHttp\Psr7\LimitStream;
use ForGravity\Fillable_PDFs\GuzzleHttp\Psr7\Request;
use ForGravity\Fillable_PDFs\GuzzleHttp\Psr7\Uri;
...
class StreamableUpload
{
     ...
     */
    public function __construct($client, RequestInterface $request, $mimeType, $data, $resumable = \false, $chunkSize = \false)
    {
        $this->client = $client;
    ...
    public function nextChunk($chunk = \false)
    {
        ...
        if ($size === null) {
            ...
            $headers = ['content-range' => 'bytes ' . $this->progress . '-' . $lastBytePos . '/' . $this->size, 'content-length' => $size, 'expect' => ''];
        }
        $request = new Request('PUT', $resumeUri, $headers, $chunk);
    ...
    private function fetchResumeUri()
    {
        ...
        if ($body) {
            $headers = ['content-type' => 'application/json; charset=UTF-8', 'content-length' => $body->getSize(), 'x-upload-content-type' => $this->mimeType, 'expect' => ''];
            if (\is_int($this->size)) {
